<template>
	<section>
		...
		<WidgetDialogdetails v-model="dialog" :details="selectedDatatoDetail" :status-colors="statusColor" :title="'Detail Kasus'">
			<template #actions>
				<v-btn v-if="selectedDatatoDetail?.Status?.toLowerCase() === 'new'" color="green" class="white--text text-capitalize" small @click="showDialogProses(selectedDatatoDetail['Nilai Kerugian'])">proses</v-btn>
			</template>
		</WidgetDialogdetails>
...
export default {
	...
	methods: {
		...
		async loadkasus(){
			try {
				const params = {
					...
					status: this.selectedOptionStatus ? this.selectedOptionStatus : 'new, process, completed, refund',
					sortOrder: this.sort,
		...
		async loadStatus() {
			try {
				const params = {
					name: 'New, Process, Refund, Completed'
				}
				this.SET_LOADING(true); 
